...
def generate_bassline_old(key, tempo, time_signature, measures, name, part, chord_progression):
    # Create a MIDI file with one track
    mf = MIDIFile(1)
    track = 0
    time = 0
...
def generate_melody_old(key, tempo, time_signature, measures, name, part, chord_progression):
    # Create a MIDI file with one track
    mf = MIDIFile(1)
    track = 0
    time = 0
...
def apply_fx_to_layer(wav_file, effect_params_file):
...
def mix_and_save(harm_filename, bass_filename, melo_filename, beat_filename, name):
    ...
    for part in song_arrangement:
        ...
        bassline = AudioSegment.from_wav(apply_fx_to_layer(bass_wav, 'bassline_fx.json'))
